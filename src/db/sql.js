...
const url = process.env.DATABASE_URL || 'mysql://root@localhost:3306/livecodings'
...
const vote = ({ todoId, userId }) => readStars.byUserIdAndTodoId(userId, todoId)
  .then(hasVoted => hasVoted
    ? deleteStar({ todoId, userId })
    : createStar({ todoId, userId }))
